...
def recommend(n, score):
    recipes = pd.read_csv(r"C:\Users\indir\OneDrive\Documents\Data\vegetarian parsed.csv")
...
def get_vegetarian_recs(ingredients, n=10, mean=False):
    model = Word2Vec.load(r"C:\Users\indir\OneDrive\Documents\Data\model_cbow.bin")
    model.init_sims(replace=True)
    if model:
        print("success")
    
    data = pd.read_csv(r"C:\Users\indir\OneDrive\Documents\Data\vegetarian parsed.csv")
...
if __name__ == "__main__":
    input = "mushroom, cheese, flour, lime"
    rec = get_vegetarian_recs(input)
    print(rec)
