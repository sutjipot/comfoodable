...
def recommend(n, score):
    recipes = pd.read_csv(r"C:\Users\indir\OneDrive\Documents\Data\gluten free parsed.csv")
...
def get_gf_recs(ingredients, n=10, mean=False):
    model = Word2Vec.load(r"C:\Users\indir\OneDrive\Documents\Data\model_cbow.bin")
    model.init_sims(replace=True)
    if model:
        print("success")
    
    data = pd.read_csv(r"C:\Users\indir\OneDrive\Documents\Data\gluten free parsed.csv")
...
if __name__ == "__main__":
    input = "spinach, potato, parmesan cheese, pesto"
    rec = get_gf_recs(input)
    print(rec)
